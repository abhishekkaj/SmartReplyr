...
class SmartReplyr_AI {
    ...
    public function __construct() {
        $this->api_key       = SmartReplyr_DB::get_setting( 'openai_api_key', '' );
        $this->model         = SmartReplyr_DB::get_setting( 'openai_model', 'gpt-4o-mini' );
        $this->system_prompt = SmartReplyr_DB::get_setting( 'system_prompt', '' );
    }
    ...
    public function get_response( $user_message, $history, $page_context, $lead ) {
        if ( empty( $this->api_key ) ) {
            return SmartReplyr_DB::get_setting( 'fallback_message', 'Our team will get back to you shortly!' );
        }

        // Search knowledge base for relevant context
        $kb_results = SmartReplyr_DB::search_kb( $user_message, 5 );
        ...
        if ( is_wp_error( $response ) ) {
            error_log( 'SmartReplyr Error: ' . $response->get_error_message() );
            return SmartReplyr_DB::get_setting( 'fallback_message', 'I\'m experiencing a technical issue. Please try again or contact our admissions team directly.' );
        }
