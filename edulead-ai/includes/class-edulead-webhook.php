...
class EduLead_Webhook {

    /**
     * Send lead data to configured webhook URL with dynamic field mapping.
     */
    public function send( $lead ) {
        $webhook_url = EduLead_DB::get_setting( 'webhook_url', '' );
        if ( empty( $webhook_url ) ) {
            return false;
        }

        // Get field mapping
        $mapping_json = EduLead_DB::get_setting( 'field_mapping', '{}' );
        ...
        if ( is_wp_error( $response ) ) {
            error_log( 'EduLead Webhook Error: ' . $response->get_error_message() );
            return false;
        }

        $code = wp_remote_retrieve_response_code( $response );
        if ( $code < 200 || $code >= 300 ) {
            error_log( 'EduLead Webhook HTTP ' . $code . ': ' . wp_remote_retrieve_body( $response ) );
            return false;
        }
