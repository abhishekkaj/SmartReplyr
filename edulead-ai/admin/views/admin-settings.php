<div class="wrap edulead-wrap">
    <h1>EduLead AI Settings</h1>
    
    <?php settings_errors( 'edulead_messages' ); ?>
    <?php $active_tab = isset( $_GET['tab'] ) ? $_GET['tab'] : 'general'; ?>
    
    <h2 class="nav-tab-wrapper">
        <a href="?page=edulead-settings&tab=general" class="nav-tab <?php echo $active_tab == 'general' ? 'nav-tab-active' : ''; ?>">General & Widget</a>
        <a href="?page=edulead-settings&tab=ai" class="nav-tab <?php echo $active_tab == 'ai' ? 'nav-tab-active' : ''; ?>">AI Engine Config</a>
        <a href="?page=edulead-settings&tab=webhook" class="nav-tab <?php echo $active_tab == 'webhook' ? 'nav-tab-active' : ''; ?>">CRM Webhook</a>
        <a href="?page=edulead-settings&tab=email" class="nav-tab <?php echo $active_tab == 'email' ? 'nav-tab-active' : ''; ?>">Email & SMTP</a>
    </h2>
    
    <form method="post" action="">
        <?php wp_nonce_field( 'edulead_settings_action', 'edulead_settings_nonce' ); ?>
        <input type="hidden" name="edulead_save_settings" value="1">
        <?php $settings = EduLead_DB::get_all_settings(); ?>
        
        <div class="el-settings-content">
            ...
            <table class="form-table">
                <tr>
                    ...
                    <td><input name="bot_name" type="text" id="bot_name" value="<?php echo esc_attr( $settings['bot_name'] ?? 'EduLead AI' ); ?>" class="regular-text"></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        <div style="display:flex; gap:10px; align-items:center;">
                            <img id="el-avatar-preview" src="<?php echo esc_url( $settings['avatar_url'] ?: EDULEAD_AI_PLUGIN_URL . 'assets/img/default-avatar.svg' ); ?>" style="width:40px; height:40px; border-radius:50%; object-fit:cover; border:1px solid #ccc;">
                            <input name="avatar_url" type="text" id="avatar_url" value="<?php echo esc_attr( $settings['avatar_url'] ?? '' ); ?>" class="regular-text">
                            <button type="button" class="button button-secondary" id="el-upload-avatar">Choose Image</button>
                        </div>
                    </td>
                </tr>
                <tr>
